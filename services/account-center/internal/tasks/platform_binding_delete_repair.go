...
func NewPlatformBindingDeleteRepairHandler(db *gorm.DB, platformService *serviceplatform.PlatformService) *PlatformBindingDeleteRepairHandler {
	...
	orchestrationService := serviceplatformbinding.NewOrchestrationService(bindingService, platformService, serviceplatformbinding.NewGRPCGenericCredentialGateway(platformService.ControlDialer()), profileService, grantService)
