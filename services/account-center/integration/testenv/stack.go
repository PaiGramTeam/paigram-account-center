...
const (
	...
	postgresUser         = "paigram_it"
	postgresPass         = "paigram_it_password"
	baselineDBName       = "paigram_it_baseline"
...
func Bootstrap(ctx context.Context, backendRoot string) (*SharedStack, error) {
	sharedM.Lock()
	...
	if err := stack.migrateBaseline(ctx, backendRoot); err != nil {
...
func (s *SharedStack) migrateBaseline(ctx context.Context, backendRoot string) error {
	...
	cfg := config.DatabaseConfig{
		DSN:           s.BaselineDSN,
		MigrationsDir: filepath.Join(backendRoot, "initialize", "migrate", "sql"),
	}
	if err := initmigrate.Run(db, cfg); err != nil {
		return fmt.Errorf("migrate baseline database: %w", err)
	}
	return nil
}

func (s *SharedStack) teardown(ctx context.Context) error {
	var result error
	if s.redis != nil {
...
func NewPerTestDB(t *testing.T) *PerTestDB {
	...
	if _, err := admin.ExecContext(ctx, fmt.Sprintf(
		...
		quoteIdentifier(name), quoteIdentifier(baselineDBName), quoteIdentifier(postgresUser),
	)); err != nil {
		_ = admin.Close()
		t.Fatalf("clone PostgreSQL baseline: %v", err)
	}
	_ = admin.Close()
