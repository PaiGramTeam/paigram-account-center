...
func TestCheckRuntimeHealthDefaultsToPlaintext(t *testing.T) {
	listener, err := net.Listen("tcp", "127.0.0.1:0")
	require.NoError(t, err)
	healthServer := health.NewServer()
	...
	require.NoError(t, checkRuntimeHealth(context.Background(), healthcheckOptions{
		Target: listener.Addr().String(), Timeout: 2 * time.Second,
	}))
}
