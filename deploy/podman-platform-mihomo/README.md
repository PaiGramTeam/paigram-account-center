...
Each deployment first stops the previous Compose project and verifies that no project container remains, then runs a digest-pinned one-shot `migrate` service after PostgreSQL becomes healthy. Platform Mihomo starts only after that job exits successfully. The migration command reads only the database DSN secret and embedded migrations; it does not weaken the runtime requirements for Redis, TLS identities, keyrings, metrics, or the HTTPS upstream. A failed teardown or migration keeps both gRPC listeners stopped; preserve the failing `deploy.ps1` output before retrying because the one-shot container is removed after execution.
